...
class Util {
  constructor() {
    ...
    this.randomServerStats = (totalRam) => {
      return {
        pid: {
          cpu: Math.random() * 20,
...
class DatabaseSQL {
  constructor(env) {
    ...
    this.saveAuthorizationToken = async (token, time) => {
      const query =
      ...
      return await fetch(`https://${APNS_HOST_NAME}/3/device/${deviceToken}`, {
        ...
        headers: JSON.parse(
          JSON.stringify({
            ...
            "apns-priority":
              headers["apns-priority"] > 0 ? headers["apns-priority"] : undefined,
...
class Handler {
  constructor(db, options) {
    ...
    this.totalRam = options.totalRam;
    ...
    this.register = async (parameters) => {
      ...
      return new Response(
        JSON.stringify({
          ...
          message: "success",
          timestamp: util.getTimestamp(),
          data: { key: key, token: deviceToken },
        }),
        {
          status: 200,
          headers: { "content-type": "application/json" },
    ...
    this.monitor = async () => {
      return new Response(JSON.stringify(util.randomServerStats(this.totalRam)), {
        status: 200,
        headers: { "content-type": "application/json" },
      });
    };
...
async function handle(request, env, ctx, db) {
  ...
  const totalRam = env.TOTAL_RAM ? parseInt(env.TOTAL_RAM) : undefined;
  const mode = env.MODE;
